...
class startMeeting(targetState: GameState, targetCharacter: String, targetPlace: String) : GameAction(targetState, targetCharacter,
    ...
    override fun chooseParams() {
        meetingName = GameEngine.acquire(tgtState.scheduledMeetings.filter { it.value.time+2 > tgtState.time && tgtState.time+2>it.value.time && it.value.place==tgtPlace}.filter { !tgtState.ongoingMeetings.containsKey(it.key) }.filter { it.value.characters.contains(tgtCharacter) }.keys.toList())
    }
