...
class BudgetResolution(override val tgtCharacter: String, override val tgtPlace: String) : GameAction()
{
    ...
    override fun isValid(): Boolean =
        parent.ongoingConferences.any { it.value.subject == "budgetResolution" } &&
                parent.ongoingConferences.filter { it.value.subject == "budgetResolution" }.values.first().currentCharacters.containsAll(
    ...
    override fun execute()
    {
        ...
        with(parent) {
            ...
            ongoingConferences.filter { it.value.subject == "budgetResolution" }.values.first().currentCharacters.forEach { characters[it]!!.frozen++ }
