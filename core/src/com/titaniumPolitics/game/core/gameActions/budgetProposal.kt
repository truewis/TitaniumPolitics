...
class budgetProposal(override val tgtCharacter: String, override val tgtPlace: String) : GameAction() {
    ...
    override fun chooseParams() {
        ...
        parent.places.forEach { if(it.key=="home" || it.value.responsibleParty=="")return@forEach else budget[it.value.responsibleParty] = (budget[it.value.responsibleParty]?:0)+ it.value.plannedWorker*(it.value.workHoursEnd-it.value.workHoursStart)*15 }
        }
