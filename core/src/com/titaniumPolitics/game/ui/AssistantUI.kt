...
class AssistantUI(gameState: GameState) : Table(defaultSkin), KTable
{
    ...
    init
    {
        ...
        gameState.onAddScheduledMeeting += { meeting ->
            Gdx.app.postRunnable {
                if (meeting.time - gameState.day*86400/ReadOnly.dt <= 5*86400/ReadOnly.dt && meeting.scheduledCharacters.contains(gameState.playerName)) {//5 days from today's start
